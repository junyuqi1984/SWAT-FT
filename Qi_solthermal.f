	...
	!!-------------------------snow thermal parameters--------------------------
	 ...
	 ca_sno(j)=1.9 * sno_den(j)**2/0.917           !!J/(cm^3.C)  1.9*10**6 sno_den/ice_den (J/kg.K)
      ...
      do k = 1, nly
       ...
       ca_sol(k,j)=4.1868*( 0.48*sol_minv(k,j) !!j/(cm^3.C)
     &             +0.6*sol_orgv(k,j)
     &             +1*sol_wcv(k,j)+0.45*sol_icev(k,j))
   ...
   !  &         + (h2osoi_ice(k,j)*cpice+ h2osoi_liq(k,j)*cpliq)  ! J/(m3 K)
	...
	!!define saturation
	 ...
	 do k = 1, nly
	 sol_satu(k,j)= max((sol_wc(k,j)+sol_ice(k,j))/sol_pormm(k,j)
     &	                       ,0.0001)
       sol_kd(k,j)=864*((0.135*sol_bd(k,j)*1000+64.7)   !! J/(cm d C)
     &            / (2700-0.947*sol_bd(k,j)*1000)+ pp)  !!~20
       end do
	 ...
	 sol_ke(k,j)= sol_satu(k,j)
       sol_ksat(k,j)=(864*2.9)**(1-sol_por(k,j))*(864*2.2)**
     &	 (sol_por(k,j)-sol_wcv(k,j))
     & 	           *(0.57*864)**sol_wcv(k,j)          !! J/(cm d C)
       ...
       sol_ksat(k,j)=(864*0.57)**sol_por(k,j)
     &               * (2.9*864)**(1-sol_por(k,j))
